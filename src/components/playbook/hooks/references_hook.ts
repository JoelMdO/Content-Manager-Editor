...
import insertLink from "../../dashboard/menu/button_menu/utils/insert_link";
...
export const useReferences = () => {
  ...
  const updateReferencePaste = async (
    index: number,
    ...
    for (const item of items) {
      if (item.type.startsWith("image/")) {
        errorAlert(
          "Url",
          "non200",
          "Files, Images are NOT allowed to be pasted."
        );
        const updatedReferences = [...references];
        updatedReferences[index] = {
          ...updatedReferences[index],
          link: "Reference not valid",
          title: updatedReferences[index].title || "",
        };
        setReferences(updatedReferences);
      } else {
