...
const CustomButton: React.FC<ButtonProps> = ({
  ...
  let text: string,
    ...
    otherFeatures: string = "font-bold mt-4";
  let textSmallSize: string = "text-[0.60rem]",
    shadow: string = "shadow-md shadow-black",
    height: string = "h-[40px]";
  let position: string;
    ...
    <>
      {isLink ? (
        ...
        <button
          type="button"
          data-cy={dataCity}
          className={`${height} ${width} ${textColor} ${otherFeatures} ${shadow} ${position} ${
            isClicked ? "text-black" : { textColor }
          } ${textSmallSize} md:text-lg rounded text-center flex items-center justify-center md:gap-2 gap-1  ${
            isClicked ? "bg-cream" : `${color}`
          } hover:${hover_color}`}
          onClick={() => {
            if (onClick) onClick();
            // setTimeout(() => {
