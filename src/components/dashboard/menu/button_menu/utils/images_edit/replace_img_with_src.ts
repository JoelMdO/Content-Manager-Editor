      ...
      if (imgSource) {
        ...
        switch (type) {
          ...
          case "html":
            ...
            break;
          default:
            console.log('"Replacing image for default type"');
            console.log("HTML content before replacement:", newHtmlContent);
            const escapedImageIdentifier = imageIdentifier.replace(
              /[.*+?^${}()|[\]\\]/g,
              "\\$&",
            );
            console.log("Escaped image identifier:", escapedImageIdentifier);
            const imageTagPattern = new RegExp(
              `<img\\s+[^>]*\\balt\\s*=\\s*(['"])${escapedImageIdentifier}\\1[^>]*>`,
              "gi",
            );
            console.log("Image tag pattern:", imageTagPattern);
            console.log("imgeSource", imgSource);

            const replacement = `<img src="${imgSource}" alt="${imageIdentifier}" width="25%"/>`;
            console.log("Replacement string:", replacement);

            const replacedHtmlContent = newHtmlContent.replace(
              imageTagPattern,
              replacement,
            );
            console.log("Default image replacement count", {
              imageIdentifier,
              replaced: replacedHtmlContent !== newHtmlContent,
            });
            newHtmlContent = replacedHtmlContent;
